...
class VideoEditor:
    ...
    TITLE_FONT = (Path(__file__).resolve().parents[2] / "assets" / "fonts" / "Montserrat-Black.ttf").as_posix().replace(":", "\\:")
        ...
        if not self.has_audio:
            ...
            try:
                from .audio_enhancer import (
                    enhance_speech, cache_key_for_audio, transform_voice,
                )
                import subprocess
                ...
                subprocess.run(
                    [
                        ...
                        "-vn", "-ac", "1", "-ar", "48000", "-c:a", "pcm_s16le", str(extracted_wav)
                    ],
                ...
                cached_wav = cache_dir / f"{audio_hash}_enhanced.wav"

                if not cached_wav.exists():
                    self._progress(6, "Limpiando audio con IA (DeepFilterNet)")
                    enhance_speech(extracted_wav, cached_wav)

                # Voice transform de calidad: pitch + formant + color (sin chipmunk).
                # Solo se aplica si el usuario eligio una voz distinta de Normal.
                final_audio = cached_wav
                voice_color = getattr(self, "_voice_color", None)
                needs_voice_transform = (
                    abs(pitch_factor - 1.0) > 0.01
                    or abs(formant_factor - 1.0) > 0.01
                    or voice_color is not None
                )
                if cached_wav.exists() and needs_voice_transform:
                    color_tag = voice_color or "none"
                    voice_wav = cache_dir / (
                        f"{audio_hash}_voice_p{pitch_factor:.2f}_f{formant_factor:.2f}_{color_tag}.wav"
                    )
                ...
                import logging
                logging.getLogger(__name__).warning("Audio enhancement failed: %s", e)
                audio_index = 0
    ...
    def _build_filter_complex(
        ...
        parts = [
            ...
            (
                ...
                f"eq=contrast={contrast:.2f}:saturation=1.04:brightness=0.00,"
                f"unsharp=5:5:{sharpen:.2f}:3:3:0.30,format=rgba[top]"
            ),
        ...
        title_start_y = 180
        title_line_height = title_font_size + 18
        for index, title_line in enumerate(title_lines):
            ...
            parts.append(
                ...
                f"shadowcolor=black@0.45:shadowx=3:shadowy=4:"
        ...
        parts.append(
            f"[{audio_index}:a]" + self._build_audio_chain(speed, volume_db, pitch_factor) + "[aout]"
        )
    ...
    def _video_encoder_args(cls) -> list[str]:
        """Devuelve args FFmpeg para video. Prioridad:
          ...
          3. H.264 NVENC universal (GPUs viejas y nuevas).
          4. Fallback CPU libx264.
        """
        import os as _os
    ...
    @staticmethod
    def _build_audio_chain(speed: float, volume_db: float, pitch_factor: float = 1.0) -> str:
        ...
        if profile in ("voice_pro", "cinema", "default"):
            # anlmdn requiere sample fmt fltp; convertimos antes para evitar
            # errores en algunos builds de FFmpeg.
            # anlmdn: denoise por non-local means - mucho mejor para ruido
            # estacionario (aire, ventiladores, calle). s=10 strength razonable.
            denoise = "aformat=sample_fmts=fltp,anlmdn=s=10:p=0.002:r=0.006,"
            ...
            target = "I=-16:TP=-1.5:LRA=11" if profile == "cinema" else "I=-14:TP=-1.5:LRA=10"
            tail = f"loudnorm={target},alimiter=limit=0.96"
            return base + denoise + notch + shaping + comp + tail
    ...
    def _pil_font_for(cls, font_size: int):
        ...
        font_file = (
            Path(__file__).resolve().parents[2] / "assets" / "fonts" / "Montserrat-Black.ttf"
        )
        try:
    ...
    def _wrap_title(cls, text: str) -> tuple[list[str], int]:
        ...
        candidate_sizes = [92, 84, 76, 68, 60, 54, 48]
